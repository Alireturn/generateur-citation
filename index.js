...
let citations = [
  ...
  let citationUser     = document.querySelector('#citation');
  ...
  nouvelleCitation.addEventListener('click', () => {
    citations.forEach(citation => {
         ...
         let randomCitation       = Math.floor(Math.random()*citations.length);
         citationUser.textContent = citations[randomCitation][0];
         auteur.textContent       = citations[randomCitation][1];

         console.log(citationUser.textContent);
    })
  });


  
//   let btnDarkc= document.getElementById('btn-3');

//   function myFunction() {
//     document.body.style.backgroundColor = "black";
//     element.classList.toggle("dark-mode");
//   }
//   btnDarkc.addEventListener('click', myFunction);